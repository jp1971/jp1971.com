...
class ECStarsRating {
	...
	public function _add_menu_page() {
		add_options_page( __('EC Stars Rating Options', self::$textdomain), 'KRNL Stars Rating', 'administrator', __FILE__, array($this, '_options_page'));
	}
	...
	<div class="ec-stars-outer<?php if($microformats) { echo ' hreview-aggregate';} ?>"<?php if( ! $microformats ) { echo ' itemscope itemtype="http://schema.org/AggregateRating"'; }?>>
		<div class="ec-stars-wrapper" data-post-id="<?php echo $post->ID ?>">
			...
			<a href="#" data-value="1" title="1/5"><i class="icon-star"></i></a>
			<a href="#" data-value="2" title="2/5"><i class="icon-star"></i></a>
			<a href="#" data-value="3" title="3/5"><i class="icon-star"></i></a>
			<a href="#" data-value="4" title="4/5"><i class="icon-star"></i></a>
			<a href="#" data-value="5" title="5/5"><i class="icon-star"></i></a>
		</div>
	...
	/**
	 * Render the stars read only
	 * @return void
	 */
	public function read_only( $id ) {
		global $post;
		// Get the post rating, votes and options
		$rating = get_post_meta($id, 'ec_stars_rating', true);
		$votes = get_post_meta($id, 'ec_stars_rating_count', true);
		$microformats = get_option('ec_stars_rating_use_microformats');

		// if the rating is an empty string (we cannot use empty because empty('0') is true) create the post meta
		if( $rating === '' ) {
			$rating = 0;
			add_post_meta($id, 'ec_stars_rating', 0);
		}
		// The same but for the votes count
		if( $votes === '' ) {
			$votes = 0;
	...
	<div class="pull-right">
		<div class="ec-stars-outer">
			<div class="ec-stars-wrapper" data-post-id="<?php echo $id; ?>">
				<div class="ec-stars-overlay" style="width: <?php echo (100 - $result * 100 / 5) ?>%"></div>
				<i class="icon-star"></i>
				<i class="icon-star"></i>
				<i class="icon-star"></i>
...
function ec_stars_read_only ( $id ) {
	global $ecStarsRating;
	$ecStarsRating->read_only( $id );
}